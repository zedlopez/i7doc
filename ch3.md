...
Unless you’re planning to have several phasers in your game, there’s
...
Most of the objects in a game will likely have the same vocabulary
...
If you want to force the player to examine the billiard table to
...
In the simplest interactive fiction games, every portable thing in
...
The words “in” and “on” mean just what you think they should – and
...
Properties can be of two types. Some of them are numbers, while
...
With simple objects such as a knife or a cup, there’s usually no need
...
My method requires a side trip to [WI 17.3: Overriding existing commands](../WI_17.html#section3), where
...
First, it implements the smelling action in a more complete way.
...
toad and moves the princess to the room where the toad was.

If we’re transforming one inanimate object into another, we’ll want
...
which I spotted while poking around in the [IF Wiki](https://ifwiki.org/)). The list of
new actions suggested by Jan Thorsby in that post was short but
suggestive: holding an object close to or against another object,
holding an object up in the air, threatening an NPC with an object, or
tipping a bookshelf forward so that the thing on the top shelf will fall
down. I’ll leave you to explore the process of threatening an NPC for
yourself. The other three actions are illustrated below.
...
The player might very reasonably want to hold an object up in the air
...
if you’re using this action in a game.

The key point in this example is that the sheet has to be held aloft
...
The details are taken care of by the after dropping something and
...
Why make aloft a general property of all things, when you’re only
...
because the torch already has that property.
...
Placing an object out of reach – on a high shelf or down at the
...
This example creates a new action, holding it against. This action
...
you have written yourself, into rulebooks. *Both* of our new
...
rulebook *until one of the rules makes a decision*. Then it will
...
Notice the non-default message that’s printed out when the player
